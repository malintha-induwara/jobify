...
import { SafeAreaView, ScrollView, Text, TouchableOpacity, View, Image, ActivityIndicator, Alert, StyleSheet, Linking } from "react-native";
import { useLocalSearchParams, useRouter } from "expo-router";
import { ArrowLeft, Briefcase, MapPin, Calendar, BookmarkPlus, ExternalLink, Check } from "lucide-react-native";
...
const JobDetails = () => {
  ...
  const handleSaveJob = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "There was a problem updating your saved jobs.");
    }
  };
      ...
      <View className="p-6 bg-white border-t border-gray-100">
        <View className="flex-row">
          <TouchableOpacity onPress={handleSaveJob} className={`flex-1 mr-3 py-4 rounded-xl flex-row justify-center items-center ${isSaved ? "bg-blue-100" : "bg-blue-50"}`} disabled={checkingSaved}>
            {checkingSaved ? <ActivityIndicator size="small" color="#0061FF" /> : isSaved ? <Check size={20} color="#0061FF" /> : <BookmarkPlus size={20} color="#0061FF" />}
